...
public class UserController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Username availability checked", content = @Content(schema = @Schema(implementation = UsernameAvailabilityResponse.class)))
    })
    ...
    /**
     * Get username by user ID (service-to-service)
     */
    @GetMapping("/users/{id}/username")
    @PreAuthorize("hasRole('SERVICE')")
    public ResponseEntity<String> getUsernameById(@PathVariable("id") UUID userId) {
    ...
    public ResponseEntity<GenericResponse> updateUsername(
        ...
        userService.updateUsername(principal.getUsername(), request);
    ...
    @Operation(summary = "Get all users (Admin only)", description = "Retrieves a list of all users in the system.", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses(value = {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Update user role (Admin only)", description = "Changes a specific user's role. Invalidates user's tokens.", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "User role updated successfully", content = @Content(schema = @Schema(implementation = RoleUpdateResponse.class))),
    ...
    public ResponseEntity<RoleUpdateResponse> updateUserRoleAdmin(
        ...
        RoleUpdateResponse response = RoleUpdateResponse.success(
                ...
                updatedUser.getRole(),
                true // Pass true directly, assuming invalidation is intended/handled elsewhere for
                // now
        );
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Toggle user ban status (Admin only)", description = "Toggles a user's ban status (banned/unbanned). Invalidates tokens if user is banned.", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "User ban status toggled successfully", content = @Content(schema = @Schema(implementation = BanStatusResponse.class))),
            @ApiResponse(responseCode = "401", description = "Unauthorized"),
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Get user by ID (Admin only)", description = "Retrieves detailed information about a specific user by ID.", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses(value = {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Delete user (Admin only)", description = "Permanently deletes a user from the system.", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "User deleted successfully", content = @Content(schema = @Schema(implementation = GenericResponse.class))),
            @ApiResponse(responseCode = "401", description = "Unauthorized"),
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Get user statistics (Admin only)", description = "Retrieves aggregated statistics about users.", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses(value = {
