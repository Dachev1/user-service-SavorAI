...
class AuthITest {
    ...
    private AuthResponse signUpAndSignIn(String username, String email, String password) throws Exception {
        ...
        // TODO: Consider a cleaner way, maybe a dedicated test endpoint or service method?
    ...
    void givenExistingUsername_whenSignup_thenConflict() throws Exception {
        ...
        RegisterRequest initialRequest = new RegisterRequest("existinguser", "initial@example.com", "Password123!");
        mockMvc.perform(post("/api/v1/auth/signup")
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        RegisterRequest duplicateUsernameRequest = new RegisterRequest("existinguser", "new@example.com", "Password456!");
    ...
    void givenExistingEmail_whenSignup_thenConflict() throws Exception {
        // Given: An existing user
        RegisterRequest initialRequest = new RegisterRequest("initialuser", "existing@example.com", "Password123!");
        mockMvc.perform(post("/api/v1/auth/signup")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(initialRequest)))
                .andExpect(status().isCreated()); // Ensure user created first
        ...
        RegisterRequest duplicateEmailRequest = new RegisterRequest("newuser", "existing@example.com", "Password456!");
    ...
    void givenValidRefreshToken_whenRefreshToken_thenOkAndNewAccessToken() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/auth/refresh-token")
                ...
                // Then: The response is 200 OK and contains a new access token and the same refresh token
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.token").isNotEmpty())
                .andExpect(jsonPath("$.refreshToken").doesNotExist());
    ...
    void givenValidAccessToken_whenLogout_thenOkAndTokenInvalidated() throws Exception {
        ...
        // And then: Verify the access token is now invalid (should ideally be blacklisted)
        // Re-configure mock for the verification step: now the token *should* be blacklisted
        when(tokenBlacklistService.isJwtBlacklisted(accessToken)).thenReturn(true);

        mockMvc.perform(post("/api/v1/auth/change-username")
                        .header("Authorization", "Bearer " + accessToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new ProfileUpdateRequest("newlogoutuser", "Password123!"))))
                .andExpect(status().isUnauthorized());
    }
